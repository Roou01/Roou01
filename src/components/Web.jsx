...
import WebGallery from "../galleries/WebGallery";
import "./Web.scss";

function Web(props) {
  const [siteClass, setSiteClass] = useState(null);
  const [designClass, setDesignClass] = useState("active");
  const [appsClass, setAppsClass] = useState(null);
  const [curGallery, setCurGallery] = useState("design");
  ...
  const siteHandler = () => {
    ...
    setDesignClass(null);
    setAppsClass(null);
    props.setOutlineClass("plus193deg");
  };
  const designHandler = () => {
    setCurGallery("design");
    setSiteClass(null);
    setDesignClass("active");
    setAppsClass(null);
    props.setOutlineClass(null);
  };
  const appsHandler = () => {
    setCurGallery("apps");
    setSiteClass(null);
    setDesignClass(null);
    setAppsClass("active");
    props.setOutlineClass("plus168deg");
  };
  ...
  return (
    <section className={"web-section " + props.webSectionOverflow}>
      <div className="gallery-holder gallery-holder-direction">
        <div className="gallery-holder__box"></div>
        <div className="scroll-line"></div>
        <WebGallery curGallery={curGallery} />
      </div>

      <div className="photo-section__text">
        <h1>WebDev.</h1>
        <p>
          Always in search for <br />
          the best solution.
        </p>
      </div>
      <div className="web-section__circle">
        <div
          className={
            "web-section__circle__circle-1 " + props.circle1WebRotation
          }
        ></div>
        <div
          className={
            "web-section__circle__circle-outline " + props.outlineClass
          }
        ></div>
        <ul className={props.webList}>
          <li onClick={siteHandler} className={siteClass}>
            Sites
          </li>
          <li onClick={designHandler} className={designClass}>
            Design
          </li>
          <li onClick={appsHandler} className={appsClass}>
            Apps
          </li>
        </ul>
        <div
          className={
            "web-section__circle__circle-2 " + props.circle2WebRotation
          }
        ></div>
        <div
          className={
            "web-section__circle__circle-3 " + props.circle3WebRotation
          }
        >
          <button className={"left-button-double " + props.webButtonDouble}>
            {rightArrow}
          </button>
          <button
            onClick={props.homeSectionActive}
            className={"left-button " + props.webButton}
          >
            {rightArrow}
          </button>
        </div>
